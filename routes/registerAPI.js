...
if(!(process.env.USER_ShowApi && process.env.USER_ShowApi=="0")){//配置，关掉api说明
  router.get('/', function(req, res, next) {
    res.render('apiShow', { title: 'api 说明' , actions: _.map(apiList,function(item){return {ctrl:item.ctrl,action:item.action}})});
  });
}
