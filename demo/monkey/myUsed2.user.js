...
(function ($, url) {
    ...
    if (/btso.pw\/search/.test(url)) {
        ...
        var count_1 = localStorage.getItem("pwCount") || 3;
        $(".data-list a").each(function (i, e) {
            if (i >= count_1)
                return;
            $.get($(e).attr("href")).then(function (res) {
                var mxStr = $(res).find("#magnetLink").text();
                console.log(mxStr);
    ...
    if (/m.zhongzimei.com\/list/.test(url)) {
        $('body').on('click', '.list-group .btnCopyMx', function (e) {
            var url = $(e.currentTarget).closest(".list-group").find("li a.text-success ").attr("href");
            $.get(url).then(function (res) {
                var mxStr = $(res).find("#copytext").text();
                console.log(mxStr);
                ...
                setTimeout(function () { return console.log("copy :" + document.execCommand('copy')); }, 100);
            });
        });
...
})(($ ? $.noConflict() : $), document.location.href);
//# sourceMappingURL=myUsed2.user.js.map